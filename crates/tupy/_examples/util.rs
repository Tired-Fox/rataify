...
use tupy::{api::{auth::OAuth, flow::AuthFlow}, Error};
...
pub async fn listen_for_authentication_code<F: AuthFlow>(flow: F, redirect: &str, auth_url: &str, state: &str) -> Result<String, Error> {
    let uri = hyper::Uri::from_str(redirect)?;
    ...
    open::that(flow.authorization_url()?)?;
