...
use crate::spotify::api::{NoContent, SpotifyResponse};
use crate::spotify::body::{StartPlayback, TransferPlayback};
use crate::spotify::response::Repeat;
use crate::spotify::Spotify;
use crate::state::{ModalWindow, Move, State, WindowState};
    ...
    pub async fn new() -> color_eyre::Result<Self> {
        ...
        tokio::task::spawn(async move {
            loop {
                ...
                // }
                actions.send(Action::Private(Private::FetchPlayback)).unwrap();
    ...
    async fn update(&mut self, action: Action) -> color_eyre::Result<()> {
        ...
        match action {
            Action::Public(public) => match public {
                            ...
                            match self.state.window.modal {
                                ModalWindow::DeviceSelect => {
                                    ...
                                    if let Some(device) = device.as_ref() {
                                        match self.spotify.transfer_playback(&TransferPlayback {
                ...
                Public::Play => call_with_device!(self.spotify.play(&StartPlayback::default()), Some(action::public!(Play))),
                Public::ToggleShuffle => call_with_device!(self.spotify.shuffle(!self.state.playback.shuffle()), None),
                Public::ToggleRepeat => call_with_device!(self.spotify.repeat(match self.state.playback.repeat() {
                    Repeat::Context => Repeat::Off,
                    Repeat::Off => Repeat::Track,
                    Repeat::Track => Repeat::Context
                }), None),
                Public::Pause => call_with_device!(self.spotify.pause(), Some(action::public!(Pause))),
                Public::TogglePlayback => match self.state.playback.playing() {
                    true => call_with_device!(self.spotify.pause(), Some(action::public!(Pause))),
                    false => call_with_device!(self.spotify.play(&StartPlayback::default()), Some(action::public!(Play))),
                }
                Public::Close => if self.state.back() {
                ...
                Public::SelectDevice => {
                    ...
                    match self.spotify.devices().await {
                        SpotifyResponse::Ok(devices) => self.state.device_select.set_devices(devices.devices),
                        _ => return Err(Report::msg("Failed to get spotify devices"))
                    }
                    let device = self.state.playback.current.as_ref().map(|p| p.device.clone());
